...
// sets the global array of seeds for seeded hashing (random numbers from 1 - p-1)
void setSeeds() {
...
// sets the global a and b arrays for prime hashing
void setAandB() {
    int aNum = 1;
...
// assigns bloom filters their arrays of hash functions
void setHashes(Bloom &bPrime, Bloom &bSeed) {
    for (int i = 0; i < a.size(); i++) {
...
// performs tests on raw hash functions
// random data and linear data
// records times per hash as well
void testHashing() {
    // ten tests
...
// writes generated indices and runtimes to csvs
// separates by type of hash function
void writeHash(std::vector<int> primeIndices, std::vector<int> primeIndicesLinear, std::vector<double> primeTime, std::vector<double> primeTimeLinear,
                std::vector<int> seedIndices, std::vector<int> seedIndicesLinear, std::vector<double> seedTime, std::vector<double> seedTimeLinear,
                std::vector<int> storedRand, std::vector<int> storedLinear) {
...
// inserts linear data to bloom filters (unused)
void insertLinearData(Bloom &bPrime, Bloom &bSeed) {
    // measures time of each hash
    auto start = std::chrono::steady_clock::now();
    auto end = std::chrono::steady_clock::now();
...
// inserts random data into bloom filters
void insertRandomData(Bloom &bPrime, Bloom &bSeed) {
    // measures time of each hash
    auto start = std::chrono::steady_clock::now();
    auto end = std::chrono::steady_clock::now();
...
// finds the false positive rates within the bloom filters
void testFalsePos(Bloom &bPrime, Bloom &bSeed) {
    int currVal = 0;
...
// performs tests on bloom filters
// c --> k --> 11 tests per k
void testBlooms() {   
    for (int i = 0; i < c.size(); i++) { // going thru all c
        ...
        for (int k = 1; k < 23; k++) {
            ...
            for (int j = 0; j < 11; j++) { // 11 tests per k value
                Bloom TPrime(m, n);
                Bloom TSeed(m, n);
                falsePosPrime = 0.0;
                falsePosSeed = 0.0;
...
// writes data from bloom filter tests to csvs
void writeBlooms() {
